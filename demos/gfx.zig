...
pub fn main() uefi.Status {
    ...
    gfx_out.blt(@as(?[*]BltPixel, @ptrCast(@constCast(&white))), uefi.protocol.GraphicsOutput.BltOperation.blt_video_fill, 0, 0, 0, 0, gfx_out.mode.info.horizontal_resolution, gfx_out.mode.info.vertical_resolution, gfx_out.mode.info.pixels_per_scan_line) catch {};

    // Fill a rectangle (100,100)->(400,300) with red
    gfx_out.blt(@as(?[*]BltPixel, @ptrCast(@constCast(&red))), uefi.protocol.GraphicsOutput.BltOperation.blt_video_fill, 0, 0, 100, 100, 300, 200, gfx_out.mode.info.pixels_per_scan_line) catch {};
