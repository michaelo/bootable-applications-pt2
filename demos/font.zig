//! zig-port of selected parts of https://github.com/dhepper/font8x8

pub const BASE_FONT_SIZE = 8;
...
const font8x8_ext_latin: [96][8]u8 = .{
...
pub fn getGlyph(ord: i32) ?[]const u8 {
    if (ord < 0)
        return null;

    const uord = @as(usize, @intCast(ord));

    if (uord <= 0x7F) {
        // Contains an 8x8 font map for unicode points U+0000 - U+007F (basic latin)
        return font8x8_basic[uord][0..];
    } else if (uord <= 0x9F) {
        // Contains an 8x8 font map for unicode points U+0080 - U+009F (C1/C2 control)
        return font8x8_control[uord - 0x80][0..];
    } else if (uord <= 0xFF) {
        // Contains an 8x8 font map for unicode points U+00A0 - U+00FF (extended latin)
        return font8x8_ext_latin[uord - 0xA0][0..];
    }
    return null;
}
